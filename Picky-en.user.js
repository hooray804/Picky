...
(function() {
    ...
    if (document.getElementById(P_HOST)) {
        window.Picky?.end();
    }
    ...
    const P = {
        ui: { host: null, shadow: null, tool: null },
        st: {
        ...
        Modal: {
            ...
            show(title, content, isHtml = false) { 
                ...
                o.innerHTML = `<div class="picky-modal-content"><div class="picky-modal-header"><span class="picky-modal-title"></span><button class="picky-icon-button" data-action="closeModal">${ICONS.close}</button></div><div class="picky-modal-body"></div></div>`; 
                o.querySelector('.picky-modal-title').textContent = title; 
                const b = o.querySelector('.picky-modal-body'); 
                if (isHtml) { b.innerHTML = content; } else { b.innerHTML = `<textarea readonly></textarea>`; b.querySelector('textarea').textContent = content; } 
                P.ui.shadow.appendChild(o); 
                this.el = o; 
                this.el.addEventListener('click', (e) => { if (e.target.closest('[data-action="closeModal"]') || e.target === this.el) { this.hide(); } }); 
        ...
        upd8() {
            ...
            if (this.ui.match) this.ui.match.textContent = `${this.st.matchCount} matches`;
            if (this.ui.disp) {
        ...
        build() {
            ...
            let host = document.getElementById(P_HOST);
            if (!host) {
        ...
        onPick(e) {
            ...
            if (cp[0] === this.ui.host || cp.includes(this.ui.tool) || cp.includes(this.Modal.el)) return;
            
            const realTarget = cp[0] === this.ui.shield ? this.getRealTarget(e) : cp[0];
        ...
        onSelStart(e) { if (e.composedPath().includes(this.ui.tool)) return; initialTouchedEl = this.getRealTarget(e); touchMoved = false; },
        ...
        onSelEnd(e) { 
            if (this.st.min) return;
            if (touchMoved || e.composedPath().includes(this.ui.tool)) return; 
        ...
        copy(asRule = false) {
            ...
            if (this.st.cfg.shadowDomSupport && root instanceof ShadowRoot) { alert("Selectors inside Shadow DOM are copied, but may not work in global stylesheets."); }
            let text = asRule ? `${window.location.hostname}##${selector}` : selector;
            navigator.clipboard.writeText(text).then(() => { const b = this.ui.tool.querySelector(asRule ? '[data-action="copyRule"]' : '[data-action="copyCSS"]'); if (!b) return; const o = b.innerHTML; b.textContent = 'Copied!'; b.classList.add('copied'); setTimeout(() => { if (this.st.autoClose) this.end(false); else { b.innerHTML = o; b.classList.remove('copied'); } }, 1200); }).catch(() => { prompt('Copy failed:', text); if (this.st.autoClose) this.end(false); });
        },
        move(pos) { this.st.pos = pos; this.ui.tool.className = `${pos} visible`; },
        getUrl() { let el = this.st.el, url = null; for (let i = 0; i < 5 && el; i++) { url = el.getAttribute('href') || el.getAttribute('src') || el.getAttribute('data-src') || el.getAttribute('data-original'); if (url) break; const bg = window.getComputedStyle(el).backgroundImage; if (bg?.includes('url')) { url = bg.match(/url\(['"]?(.*?)['"]?\)/)[1]; if(url) break; } el = this.getParent(el); } if (url) prompt("Extracted URL:", new URL(url, window.location.href).href); else alert("URL not found."); },
        getAttr() { const a = prompt('Enter attribute name to extract (e.g., data-id, alt):'); if (!a) return; const v = this.st.el?.getAttribute(a); if (v !== null) prompt(`'${a}' Attribute value:`, v); else alert(`'${a}' Attribute not found.`); },
        showChildSel() {
            const children = this.getChildren(this.st.el); if (!children || children.length === 0) { alert('No child elements found.'); return; }
            ...
            this.Modal.show('Select Child Element', `<ul class="picky-child-list">${items}</ul>`, true);
        ...
        showCodeInspector() {
            ...
            const getRelatedJS = () => { let jsText = `/* --- Inline Event Handlers --- */\n`; let hasInline = false; for (const attr of el.attributes) { if (attr.name.startsWith('on')) { jsText += `${attr.name}="${attr.value}"\n`; hasInline = true; } } if (!hasInline) jsText += '없음\n'; jsText += `\n/* --- Inline Script Related Code based on ID or Class --- */\n`; let foundScripts = ''; const searchTerms = [el.id, ...Array.from(el.classList).filter(c => !toolClasses.includes(c))].filter(Boolean); if (searchTerms.length > 0) { const regex = new RegExp(searchTerms.map(t => CSS.escape(t)).join('|'), 'i'); document.querySelectorAll('script:not([src])').forEach((script, i) => { if (regex.test(script.innerHTML)) { foundScripts += `\n// Found in #${i+1}:\n${script.innerHTML.substring(0, 1000).trim()}...\n`; } }); } jsText += (foundScripts || '없음\n'); jsText += `\n/* Use browser devtools for external scripts or dynamic listeners. */`; return jsText; };
            const content = `<div class="picky-code-tabs"><div class="picky-code-tab active" data-tab="html">HTML</div><div class="picky-code-tab" data-tab="css">CSS</div><div class="picky-code-tab" data-tab="js">JS</div></div><div class="picky-code-panel active" data-panel="html"><pre>${getRelatedHTML().replace(/</g, '&lt;')}</pre></div><div class="picky-code-panel" data-panel="css"><pre>${getRelatedCSS().replace(/</g, '&lt;')}</pre></div><div class="picky-code-panel" data-panel="js"><pre>${getRelatedJS().replace(/</g, '&lt;')}</pre></div>`; this.Modal.show('Related Code Inspector', content, true); const modal = this.Modal.el; modal.querySelectorAll('.picky-code-tab').forEach(tab => { tab.addEventListener('click', () => { modal.querySelector('.picky-code-tab.active').classList.remove('active'); modal.querySelector('.picky-code-panel.active').classList.remove('active'); tab.classList.add('active'); modal.querySelector(`.picky-code-panel[data-panel="${tab.dataset.tab}"]`).classList.add('active'); }); });
        },
        showSrc(type) { let t = '', c = ''; switch(type) { case 'html': t = 'HTML (Current DOM)'; c = document.documentElement.outerHTML; break; case 'css': t = 'CSS (Internal Styles)'; c = `/* Only same-origin stylesheets and inline styles are displayed. */\n\n`; Array.from(document.styleSheets).forEach(s => { try { if (!s.href || s.href.startsWith(location.origin)) { c += `/* --- ${s.href || 'Inline'} --- */\n`; Array.from(s.cssRules).forEach(r => c += r.cssText + '\n'); } } catch (e) {} }); break; case 'js': t = 'JavaScript'; c = `/* List of scripts loaded on the page. */\n\n`; Array.from(document.scripts).forEach((s, i) => { c += s.src ? `<!-- External #${i+1}: ${s.src} -->\n<script src="${s.src}"></script>\n\n` : `<!-- Inline #${i+1} -->\n<script>${s.innerHTML}</script>\n\n`; }); break; } this.Modal.show(t, c); },
        showCookies() { const getCookies = () => document.cookie.split(';').filter(Boolean).map(c => { const parts = c.trim().split('='); return { name: parts[0], value: decodeURIComponent(parts.slice(1).join('=')) }; }); const render = () => { const cookies = getCookies(); if (cookies.length === 0) { return 'No non-HttpOnly cookies to display.'; } const rows = cookies.map(c => `<tr><td>${c.name}</td><td>${c.value}</td><td class="cookie-actions"><button data-cookie-name="${c.name}" data-action="editCookie">Edit</button><button data-cookie-name="${c.name}" data-action="deleteCookie" class="delete">Delete</button></td></tr>`).join(''); return `<p style="font-size:11px; color:var(--pk-on-surf-var); margin-bottom:10px;">Cookies with the HttpOnly flag are not accessible due to security policies.</p><table class="picky-cookie-table"><thead><tr><th>Name</th><th>Value</th><th>Actions</th></tr></thead><tbody>${rows}</tbody></table>`; }; this.Modal.show('Cookie Information', render(), true); this.Modal.el.querySelector('.picky-modal-body').addEventListener('click', e => { const btn = e.target.closest('button[data-cookie-name]'); if (!btn) return; const name = btn.dataset.cookieName, action = btn.dataset.action; if (action === 'editCookie') { const current = getCookies().find(c => c.name === name)?.value || ''; const newValue = prompt(`'${name}' Enter new value for cookie:`, current); if (newValue !== null) { document.cookie = `${name}=${encodeURIComponent(newValue)};path=/;max-age=31536000`; this.Modal.el.querySelector('.picky-modal-body').innerHTML = render(); } } else if (action === 'deleteCookie') { if (confirm(`'${name}' Are you sure you want to delete this cookie?`)) { document.cookie = `${name}=;path=/;expires=Thu, 01 Jan 1970 00:00:00 GMT`; this.Modal.el.querySelector('.picky-modal-body').innerHTML = render(); } } }); },
