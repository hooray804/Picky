...
(function() {
    ...
    const P = {
        ...
        viewSel() { const s = this.getSliderCfg(); const slider = `<div id="picky-nav-slider-container"><label for="picky-nav-slider" style="font-size:11px; color:var(--pk-on-surf-var)">요소 탐색 (상위 ← → 하위)</label><input type="range" id="picky-nav-slider" min="${s.min}" max="${s.max}" value="${s.val}"></div>`; return `<div class="picky-header"><div class="picky-header-title">요소 선택됨</div><div class="picky-header-actions"><button class="picky-icon-button" data-action="inspectCode" title="연관 코드 보기">${ICONS.code}</button><button class="picky-icon-button" data-action="showSettings" title="설정">${ICONS.settings}</button><button class="picky-icon-button" data-action="cycleSize" title="모드 전환">${ICONS.modeCycle}</button><button class="picky-icon-button" data-action="close" title="닫기">${ICONS.close}</button></div></div><div class="picky-selector-box"><div class="picky-selector-box-title"><span>CSS 선택자</span><span class="picky-match-count"></span></div><div class="picky-selector-display"></div></div>${slider}<div class="picky-button-grid" style="grid-template-columns: repeat(6, 1fr); margin-top: 10px; gap: 6px;"><button data-action="selParent">상위</button><button data-action="selChild">하위</button><button data-action="toggleHide">${this.st.hidden?'복원':'숨김'}</button><button data-action="permanentBlock" style="color:#fff; background-color:var(--pk-err);">차단</button><button data-action="toggleIsolate">${this.st.isolate?'해제':'격리'}</button><button data-action="selSimilar">유사</button><button data-action="extractUrl">URL</button><button data-action="extractAttr">속성</button><button data-action="copyCSS" class="primary">CSS</button><button data-action="copyRule" class="primary">규칙</button><button data-action="reset">리셋</button></div>`; },
        ...
        viewSet() { 
            ...
            return `<div class="picky-header"><button class="picky-icon-button" data-action="showSelected">${ICONS.back}</button><div class="picky-header-title">설정</div><div class="picky-header-actions"><button class="picky-icon-button" data-action="showSelected">${ICONS.close}</button></div></div>
            <div class="picky-setting-item"><span>복사 후 자동 닫기</span><label class="picky-switch"><input type="checkbox" data-action="toggleAutoClose" ${this.st.autoClose ? 'checked' : ''}><span class="picky-slider"></span></label></div>
            <div class="picky-setting-title">선택자 생성 규칙</div>
            <div class="picky-setting-item"><span>지능형 모드</span><label class="picky-switch"><input type="checkbox" data-cfg-key="intelligentMode" ${c.intelligentMode ? 'checked' : ''}><span class="picky-slider"></span></label></div>
            <div class="picky-manual-settings" ${manual}><div class="picky-setting-item"><span>ID 사용 (#id)</span><label class="picky-switch"><input type="checkbox" data-cfg-key="useId" ${c.useId ? 'checked' : ''}><span class="picky-slider"></span></label></div><div class="picky-setting-item"><span>클래스 사용 (.class)</span><label class="picky-switch"><input type="checkbox" data-cfg-key="useClasses" ${c.useClasses ? 'checked' : ''}><span class="picky-slider"></span></label></div><div class="picky-setting-item"><span>순서 사용 (:nth-of-type)</span><label class="picky-switch"><input type="checkbox" data-cfg-key="useNthOfType" ${c.useNthOfType ? 'checked' : ''}><span class="picky-slider"></span></label></div></div>
            <div class="picky-setting-title">고급 기능</div>
            <div class="picky-setting-item"><span>Shadow DOM 호환성</span><label class="picky-switch"><input type="checkbox" data-cfg-key="shadowDomSupport" ${c.shadowDomSupport ? 'checked' : ''}><span class="picky-slider"></span></label></div>
            <div class="picky-setting-title">광고 차단 관리</div>
            <div class="picky-button-grid" style="margin-top:8px; grid-template-columns: 1fr 1fr;"><button data-action="showBlockRules">규칙 보기</button><button data-action="resetBlocks" style="background-color: var(--pk-err); color: #fff;">규칙 초기화</button></div>
        ...
        act(e) {
            ...
            const actions = {
                ...
                permanentBlock: () => {
                    ...
                    if (selector) {
                        if (confirm(`다음 선택자를 영구적으로 차단(숨김)하시겠습니까?\n\n${selector}\n\n* "설정" 메뉴에서 차단을 해제할 수 있습니다.`)) {
                            this.Blocker.add(selector);
                            this.act({ target: { closest: () => ({ dataset: { action: 'reset' } }) } });
                        }
                    } else {
                        alert('선택할 수 없는 요소입니다.');
                    }
                },
                showBlockRules: () => {
                    const renderList = () => {
                        ...
                        const items = rules.map(r => `
                            <li style="display:flex; justify-content:space-between; align-items:center; gap:10px; padding:10px; border-bottom:1px solid var(--pk-outl);">
                                ...
                                <button data-rule="${r}" style="background-color:var(--pk-err); color:#fff; padding:4px 8px; font-size:11px; border-radius:8px; flex-shrink:0;">삭제</button>
                            </li>`).join('');
                        return `<ul class="picky-child-list" style="max-height:50vh; overflow-y:auto; padding:0;">${items}</ul>`;
                    };
                    this.Modal.show('현재 차단 규칙 관리', renderList(), true);
                ...
                resetBlocks: () => {
                    if (confirm('현재 사이트(' + window.location.hostname + ')에 설정된 모든 차단 규칙을 삭제하시겠습니까?')) {
                        this.Blocker.reset();
                    }
                },
                // ---------------------------
